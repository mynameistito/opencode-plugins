...
The deployment workflow uses `mynameistito/alchemy-deploy` `v2.3.1`, pinned to its full release SHA. Production runs only after successful CI on `main`; same-repository pull requests receive isolated `pr-<number>` stages and are destroyed when closed.
